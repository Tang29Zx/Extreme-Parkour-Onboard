...
-> policy: all joints <=0.20 rad/cycle, continuous constraints
```
...
推导出的PD力矩可行区间求交。若且仅若上一目标已因最新q/dq变得预测超力矩、
普通步长是唯一阻断力矩安全目标的边界，稳态hip/thigh可重算`0.30 rad`逃逸区间；
逃逸目标还必须更接近实测q并降低预测力矩绝对值。calf和首1秒接入不启用逃逸。
...
## PD力矩安全逃逸验证（2026-07-29）

`constrain_policy_target()`新增显式可选的`escape_max_step_rad`，默认关闭。生产控制
只在稳态传入`(0.30, 0.30, 0.20) × 4`，接入阶段传入`None`。普通交集为空后，代码
独立计算机械/PD力矩安全区间和逃逸步长区间，并要求上一目标按最新q/dq预测已超力矩、
逃逸候选更接近实测q且预测力矩绝对值下降；任何剩余不可行关节仍抛出原异常并进入
motor-off。控制节点只在实际步长越过普通阈值时记录关节和逃逸前后预测力矩。

使用20:16:44最终NPZ中`RR_thigh_joint`的故障字段精确重放：requested/previous/
measured q/dq为`-0.200000/-0.013357/+0.796217/+2.600477`时，输出目标为
`+0.268728925 rad`，步长`+0.282085925 rad`，预测PD力矩由`-34.983437`降至
`-23.700000 Nm`；目标同时满足机械范围、0.30逃逸步长和名义力矩边界。Python 3.8.20
下70项全套单元测试、相关语法检查和`git diff --check`通过。测试覆盖原故障、接入禁用、
calf禁用以及需要超过0.30时继续拒绝；尚未同步Jetson，也未连接ROS、相机或真实LowCmd。

## 速度越界诊断与运行时候选结论（2026-07-29）

先扩展`validate_policy_runtime_inputs()`的错误详情：位置越界按
`lower-0.05/upper+0.05 rad`，速度越界按`limit×1.005`判定，异常逐关节记录
`joint/measured/limit/threshold`，判定逻辑不变。

详细基线确认故障主要发生在箱体接触造成的calf负向速度峰值。把制动直接加入目标
区间会在calf已处于机械目标上界时形成空交集；提高运行时Kd即使通过等效PD目标正确
作用于PhysX，也会改变落箱接触相位。完整20环境实验中，全关节Kd×1.2使0.45 m从
`3成功/14硬停/3超时`恶化为`0/19/1`，只处理FL calf负向速度也恶化为`3/17/0`。

因此生产`constrain_policy_target()`、LowCmd Kd和评测CLI均不保留速度制动或动态
阻尼分支。修复转移到训练仓库：速度惩罚从额定值80%开始，位置惩罚使用URDF经
`soft_dof_pos_limit`收缩后的区间。重新训练前，当前traced权重的闭环结果不会改变。
任何额定值、0.5%测量容差或力矩边界均不得为了提高通过率而放宽。
