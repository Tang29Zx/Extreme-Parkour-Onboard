...
- 短按并释放 **Y** 进入策略；前1秒以五次曲线接入且每周期最多变化0.05 rad，
  第1秒相对起点还限制在0.3 rad以内。接入完成后hip/thigh通常以
  0.21 rad/周期、calf以0.20 rad/周期持续限制目标变化，并继续执行机械关节范围和
  估算PD力矩约束。若上一hip/thigh目标因最新q/dq已变得预测超力矩，且普通步长是
  进入力矩安全区间的唯一阻碍，单周期可条件式放宽到0.30 rad；目标必须更接近实测q
  并降低预测力矩绝对值。该逃逸不用于calf和首1秒接入，0.30后仍无解则继续硬停止。
  每次实际逃逸会写入ROS warning。策略观测中的`last_actions`保持上一帧actor原始输出。
  接入后不根据姿态自动motor-off。
...
输入检查、接入期0.05 rad/周期、稳态全关节0.20 rad/周期、
机械限位和估算PD力矩约束。
